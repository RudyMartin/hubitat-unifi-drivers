...
* 
* Thank you(s):
* @mavrrick58 for WebHook processing idea and code
...
def ApplyWebHook( String DeviceID, String WebHookType, String WebHookValue = null ){
    ...
    switch( WebHookType ) {
        ...
        case "Motion":
            ...
            PostEventToChild( "${ DeviceID }", "AIRecognitionType" , "${ WebHookType }" , null, true )
        	PostEventToChild( "${ DeviceID }", "AIRecognitionValue" , "${ WebHookValue }" , null, true )
    	break
    default:
...
def CloseWebSocket(){
    try{
        ProcessState( "WebSocketOpen", "closed" )
        pauseExecution( 500 )
            ...
            if( Data.dataPacket.dataPayload != null ){
                Data.dataPacket.dataPayload.each(){
                    switch( it.key ){
                        case "lastMotion":
                            if( it.value != null ){
                                PostEventToChild( "${ Device }", "LastMotion", ConvertEpochToDate( "${ it.value }" ) )
                            }
                            break
                        case "lastRing":
                            if( it.value != null ){
						...
						case "isRecording":
							if( it.value ){
								PostEventToChild( "${ Device }", "RecordingNow", "true", null, true )
							} else {
								PostEventToChild( "${ Device }", "RecordingNow", "false", null, true )
							}
                            if( Data.actionPacket?.actionPayload?.action?.toString() == "add" ){
							    ProcessState( "LastWSSAction", "RecordingNow" )
                            }
                            PostStateToChild( "${ Device }", "LastWSSAction", "RecordingNow" )
							break
						case "eventStats":
							if( it.value.motion.today != null ){
								PostEventToChild( "${ Device }", "MotionEventsToday", it.value.motion.today )
							}
							break
                        case "type":
                            switch( it.value ){
				...
				if( LastActionID == EventID ){
					switch( LastAction ){
						...
						case "Light":
							...
							break
						case "Recording Now":
							PostEventToChild( "${ Device }", "RecordingNow", "false" )
							break
                        case "motion":
                            PostEventToChild( "${ Device }", "motion", "inactive" )
							break
...
def webSocketStatus( String Response ){
    ...
    if( Response.startsWith( "status: open" ) ){        
        ...
        if( state.WebSocketFailures > 5 ){
            ProcessState( "WebSocketStatus", "Failed - Requires a manual login to reset" )
        } else {
            ProcessState( "WebSocketStatus", "Error" )
            reinitialize()
        }
...
def reinitialize(){
    ...
    def delayCalc = ( state.WebSocketDelay ?: 1 ) * 2    
    // upper limit is 600s
    def reconnectDelay  = delayCalc <= 600 ? delayCalc : 600
    
    ProcessState( "WebSocketDelay", reconnectDelay  )
    runIn( reconnectDelay, initialize )
}

def initialize(){
    if( EnableWebSocket && ( state.WebSocketFailures < 5 ) ){
...
def GetSnapshot( String CameraID, String DNI = null ){
    ...
    try{
        httpGet( Params ){ resp ->
            switch( resp.getStatus() ){
                case 200:
                    ProcessState( "Status", "${ Method } successful." )
                	if( resp.data ){
                        ...
                        PostEventToChild( "${ DNI }", "ImageAsOf", new Date(), null, true )
                    	PostEventToChild( "${ DNI }", "image", "http://${ location.hub.localIP }/local/${ Filename }", null, true )
                        PostEventToChild( "${ DNI }", "imageUrl", "http://${ location.hub.localIP }/local/${ Filename }", null, true )
                        PostStateToChild( "${ DNI }", "Thumbnail", "<img width=\"10%\" height=\"10%\" src=\"http://${ location.hub.localIP }/local/${ Filename }\">" )
                    } else {
                        Logging( "${ Method } response is blank", 3 )
                    }
                    break
                case 400: // Bad request
                    ProcessState( "Status", "${ Method } Bad Request" )
                    Logging( "Bad Request for ${ Method }", 5 )
                    break
                case 401:
                    ProcessState( "Status", "${ Method } Unauthorized, please Login again" )
                    Logging( "Unauthorized for ${ Method } please Login again", 5 )
                    break
                case 404:
                    ProcessState( "Status", "${ Method } Page not found error" )
                    Logging( "Page not found for ${ Method }", 5 )
                    break
                case 408:
                    ProcessState( "Status", "Request timeout for ${ Method }" )
                    Logging( "Timeout for ${ Method } headers = ${ resp.getHeaders() }", 4 )
                    break
                default:
                    ProcessState( "Status", "Error ${ resp.status } connecting for ${ Method }" )
...
def GenerateProtectImageParams( String Path, String Data = null ){
	...
	if( Controller == "Unifi Dream Machine (inc Pro)" ){
        if( Data != null ){
            Params = [ uri: "https://${ UnifiURL }/proxy/protect/${ Path }", ignoreSSLIssues: true, requestContentType: "application/octet-stream", headers: [ Host: "${ UnifiURL }", Accept: "*/*", Cookie: "${ state.Cookie }", 'X-CSRF-Token': "${ state.CSRF }", 'accessKey': "${ state.'Auth Key' }" ], data:"${ Data }" ]
        } else {
            Params = [ uri: "https://${ UnifiURL }/proxy/protect/${ Path }", ignoreSSLIssues: true, requestContentType: "application/octet-stream", headers: [ Host: "${ UnifiURL }", Accept: "*/*", Cookie: "${ state.Cookie }", 'X-CSRF-Token': "${ state.CSRF }", 'accessKey': "${ state.'Auth Key' }" ] ]
        }
	} else {
        if( Data != null ){
            Params = [ uri: "https://${ UnifiURL }:${ ControllerPort }/${ Path }", ignoreSSLIssues: true, requestContentType: "application/octet-stream", headers: [ Host: "${ UnifiURL }", Accept: "*/*", Cookie: "${ state.Cookie }", 'X-CSRF-Token': "${ state.CSRF }", 'accessKey': "${ state.'Auth Key' }" ], data:"${ Data }" ]
        } else {
            Params = [ uri: "https://${ UnifiURL }:${ ControllerPort }/${ Path }", ignoreSSLIssues: true, requestContentType: "application/octet-stream", headers: [ Host: "${ UnifiURL }", Accept: "*/*", Cookie: "${ state.Cookie }", 'X-CSRF-Token': "${ state.CSRF }", 'accessKey': "${ state.'Auth Key' }" ] ]
        }
	}
    Logging( "Parameters = ${ Params }", 4 )
	return Params
...
def PostEventToChild( Child, Variable, Value, Unit = null, ForceEvent = null, Description = null ){
    if( Child != null ){
        ...
        if( getChildDevice( Child ) != null ){
            if( ForceEvent ){
            	getChildDevice( Child ).sendEvent( name: Variable, value: Value, unit: Unit, isStateChange: true, descriptionText: Description )
